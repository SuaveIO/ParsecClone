...
type State<'StateType, 'ConsumeType, 'UserState> =
  IStreamP<'StateType, 'ConsumeType, 'UserState>

type Reply<'Return, 'StateType, 'ConsumeType, 'UserState> =
  'Return option * State<'StateType, 'ConsumeType, 'UserState>

type Parser<'Return, 'StateType, 'ConsumeType, 'UserState> =
  State<'StateType, 'ConsumeType, 'UserState> -> Reply<'Return, 'StateType, 'ConsumeType, 'UserState>
...
module Combinator =
  ...
  let getAltReply first second inputState =
    ...
    | Some(_) ->
      reply
    | None  when state.equals inputState ->
      second inputState
    | None ->
      failwith "No match found and underlying state was modified"
  ...
  let getBacktrackReply current (next:OptimizedClosures.FSharpFunc<_,_,_>) input =
    ...
    match result with
    | Some r ->
      try next.Invoke(r, state)
      with _ ->
        state.backtrack()
        reply
    | None when not (state.equals input) ->
      failwith "No match found and underlying state was modified"
    | None ->
      None, state
...
// module Operators =
  let inline private adapt x =
    OptimizedClosures.FSharpFunc<_,_,_>.Adapt(x)

  let (>>=) (current) (next) =
    getReply current (adapt next)

  let (>>=?) (current) (next) =
    getBacktrackReply current (adapt next)
  ...
  let (>>|.) parser1 transformer =
    parser1  >>= fun result ->
    preturn (transformer result)
  ...
  *)
  ...
  let many parser =
    takeWhile (fun s -> true) parser

  let many1 parser =
    takeTillWithCount 1 (fun s -> false) parser

  let anyOf comb =
    List.fold (fun acc value -> acc <|> comb value) pzero

  let choice parsers =
    List.foldBack (fun acc value -> acc <|> value) parsers pzero

  let between starting parser ending =
    starting >>. parser .>> ending

  let between2 bookend parser =
    between bookend parser bookend
  ...
  let (.<?>>.) parser listParser =
    optWith (opt parser) (opt listParser)

  let (.<<?>.) parser listParser =
    optWith (opt listParser) (opt parser)
  ...
  let createParserForwardedToRef ()=
    let refParser = ref (fun _ -> failwith "Forwarded ref parser was never initialized")
  ...
  let getUserState (state:State<_,_,_>) =
    Some (state.getUserState()), state

  let setUserState value (state:State<_,_,_>) =
    Some (state.setUserState value), state
  ...
  type ParserCombinator() =
    member __.Delay getParser = fun (stream:State<_,_,_>) -> getParser() <| stream
    member __.Return(x) = preturn x
    member __.Bind(parser, next) = parser >>= next
    member __.Zero() = pzero
    member __.ReturnFrom p = p

    member __.TryWith parser (customFail: exn -> Parser<_,_,_,_>) =
      fun stream ->
          try parser stream with exn -> (customFail exn) stream

    member __.TryFinally parser finalBlock =
      fun stream ->
